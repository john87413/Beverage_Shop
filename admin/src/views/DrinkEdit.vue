  ...
  <div>
    ...
    <el-form label-width="120px" @submit.native.prevent="save">
      <el-tabs value="basic" type="border-card">
        <el-tab-pane label="飲料資訊" name="basic">
  ...
  methods: {
    ...
    async save() {
      if (this.model.name != null) {
        if (this.id) {
          await this.$http.put(`rest/drink/${this.id}`, this.model);
        } else {
          await this.$http.post("rest/drink", this.model);
        }
        this.$router.push("/drink/list"); //如果是要到某個url的話前面要加 '/' 不然會很奇怪
    ...
    // 編輯時要拿到編輯之前的資料
    async fetch() {
      const res = await this.$http.get(`rest/drink/${this.id}`);
